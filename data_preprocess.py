...
def extract_noise():
    ...
    for i, path in enumerate(os.listdir(clean_path)):
        clean, sr = librosa.core.load(os.path.join(clean_path, path), sr=sr)  # load clean audio
...
def save_spectrogram_tisv(i):
    ...
    for j, folder in enumerate(os.listdir(config.dataset_audio_path)):
        ...
        np.save(os.path.join("../speaker_verification_spectrograms", folder +".npy"), utterances_spec)
        #else:
...
def save_spectrogram_tisv_for_each_wav():
    ...
    for j, folder in enumerate(os.listdir(config.dataset_audio_path)):
        ...
        for utter_name in os.listdir(speaker_path):
            ...
            #print(utterances_spec.shape)
        #if i<train_speaker_num:      # save spectrogram as numpy file
            np.save(os.path.join("../speaker_verification_spectrograms", folder+"_"+utter_name.strip('.wav').strip(".m4a")+".npy"), utterances_spec)
            #k=k+1
        #else:
        #    np.save(os.path.join(config.test_path, "speaker%d.npy"%(i-train_speaker_num)), utterances_spec)
